...
test("local manifest accepts compatible exact nonsecret schemas", () => {
  ...
  assert.deepEqual(parseManifest({ ...valid, cli_version: "0.1.4" }), {
    ...valid,
    cli_version: "0.1.4",
  });
  assert.deepEqual(parseManifest({ ...valid, cli_version: "0.2.0" }), {
    ...valid,
    cli_version: "0.2.0",
  });
  assert.deepEqual(parseManifest({ ...valid, cli_version: "0.2.1" }), {
    ...valid,
    cli_version: "0.2.1",
  });
  assert.equal(parseManifest({ ...valid, cli_version: "0.2.2" }), null);
